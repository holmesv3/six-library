...
{
    std::string result =
            six::XMLControl::dataTypeToString(six::DataType::COMPLEX);
    TEST_ASSERT_EQ("SICD_XML", result);

    result = six::XMLControl::dataTypeToString(six::DataType::DERIVED,
                                               false /*appendXML*/);
    ...
    *pIntDataType = 999;  // bypass overloads; should now be a garbage value
    ...
    TEST_EXCEPTION(six::XMLControl::dataTypeToString(
            dataType));  // the "default:" case label will throw, as desired
}

TEST_CASE(testXmlLiteAttributeClass)
{
    six::XmlLite xmlLite(xml::lite::Uri("urn:example.com"),
                         true /*addClassAttributes*/);
    ...
    {
        auto* e = xmlLite.createBooleanType(xml::lite::QName("Boolean"),
...
{
    // This doesn't have anything to do with XML per-se, but that's the main
    // use-case for six::toString()
...
void test_six_toType_NOT_SET(const std::string& testName)
{
    ...
    const auto v = six::toType<T>(
            "Q W E R T Y");  // any string that will cause failure
...
{
    test_six_toType_<T>(testName);
    TEST_EXCEPTION(six::toType<T>(
            "Q W E R T Y" /*any string that will cause failure*/));  // throw
                                                                     // rather
                                                                     // than
                                                                     // returning
                                                                     // NOT_SET
...
TEST_MAIN(TEST_CHECK(loadCompiledSchemaPath); TEST_CHECK(respectGivenPaths);
