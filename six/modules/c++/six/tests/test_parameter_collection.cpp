...
int main()
{
    ...
    if (pc.end() - pc.begin() != pv.end() - pv.begin() || *pc.begin() != p0)
    {
    ...
    six::ParameterCollection::ParameterCollectionIteratorT find_non_member =
            std::find(pc.begin(), pc.end(), non_member);
    if (find_p6 != p6 || find_non_member != pc.end())
    {
        std::cout << "Does not seem to work with standard algorithms"
                  << std::endl;
        return 1;
    }
