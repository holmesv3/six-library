...
struct SIX_SIX_API SICommonXMLParser : public XMLParser
{
    ...
    XMLElem createComplex(const std::string& name,
                          six::zdouble c,
    ...
    XMLElem createVector2D(const std::string& name,
                           Vector2 p = 0.0,
                           XMLElem parent = nullptr) const;
    XMLElem createVector3D(const std::string& name,
                           const std::string& uri,
                           Vector3 p = 0.0,
                           XMLElem parent = nullptr) const;
    XMLElem createVector3D(const std::string& name,
                           Vector3 p = 0.0,
                           XMLElem parent = nullptr) const;
    XMLElem createRowCol(const std::string& name,
                         const std::string& uri,
    ...
    XMLElem createLatLon(const std::string& name,
                         ...
                         const LatLon& value,
                         XMLElem parent = nullptr) const;
    XMLElem createLatLonAlt(const std::string& name,
                            const LatLonAlt& value,
                            XMLElem parent = nullptr) const;

    XMLElem createPoly1D(const std::string& name,
                         const std::string& uri,
                         const Poly1D& poly1D,
                         XMLElem parent = nullptr) const;
    XMLElem createPoly2D(const std::string& name,
                         const std::string& uri,
                         const Poly2D& poly2D,
                         XMLElem parent = nullptr) const;
    XMLElem createPoly1D(const std::string& name,
    ...
    XMLElem createPoly2D(const std::string& name,
                         const Poly2D& poly2D,
                         XMLElem parent = nullptr) const;
    XMLElem createPolyXYZ(const std::string& name,
                          const PolyXYZ& polyXYZ,
                          XMLElem parent = nullptr) const;
    XMLElem createParameter(const std::string& name,
                            const std::string& uri,
    ...
    void addParameters(const std::string& name,
                       const ParameterCollection& props,
                       XMLElem parent = nullptr) const;
    void addDecorrType(const std::string& name,
                       const std::string& uri,
                       DecorrType dt,
                       XMLElem p) const;
    void addDecorrType(const std::string& name,
                       const std::string& uri,
                       const std::optional<DecorrType>& dt,
                       XMLElem p) const;
    ...
    XMLElem convertGeoInfoToXML(const GeoInfo& geoInfo,
                                bool hasSIPrefix,
                                XMLElem parent = nullptr) const;
    void parseGeoInfoFromXML(const xml::lite::Element* geoInfoXML,
                             GeoInfo* geoInfo) const;

    void parseEarthModelType(const xml::lite::Element* element,
    ...
    bool parseOptionalPoly2D(const xml::lite::Element*,
                             const std::string&,
                             Poly2D&) const;
    void parsePolyXYZ(const xml::lite::Element* polyXML,
    ...
    void parseLatLon(const xml::lite::Element& parent, LatLon& ll) const;
    void parseLatLons(const xml::lite::Element* pointsXML,
                      const std::string& pointName,
                      std::vector<LatLon>& llVec) const;
    void parseRangeAzimuth(const xml::lite::Element* parent,
                           types::RgAz<double>& value) const;

    void parseRowColDouble(const xml::lite::Element* parent,
                           const std::string& rowName,
                           const std::string& colName,
    ...
    void parseRowColInt(const xml::lite::Element* parent,
                        const std::string& rowName,
                        const std::string& colName,
                        RowColInt& rc) const;
    void parseRowColInt(const xml::lite::Element* parent, RowColInt& rc) const;
    void parseRowColInts(const xml::lite::Element* pointsXML,
                         const std::string& pointName,
                         std::vector<RowColInt>& rcVec) const;
    void parseParameter(const xml::lite::Element* element,
                        Parameter& param) const;

    void parseRowColLatLon(const xml::lite::Element* parent,
                           RowColLatLon& rc) const;

    void parseParameters(const xml::lite::Element* paramXML,
                         const std::string& paramName,
                         ParameterCollection& props) const;

    void parseDecorrType(const xml::lite::Element* decorrXML,
                         DecorrType& decorrType) const;
    void parseOptionalDecorrType(const xml::lite::Element* parent,
    ...
    void parseFootprint(const xml::lite::Element&,
                        const std::string& cornerName,
                        LatLonCorners&) const;
    ...
    XMLElem convertErrorStatisticsToXML(const ErrorStatistics* errorStatistics,
                                        XMLElem parent = nullptr) const;

    void parseErrorStatisticsFromXML(const xml::lite::Element* errorStatsXML,
                                     ErrorStatistics* errorStatistics) const;
    void parseErrorStatisticsFromXML(const xml::lite::Element& errorStatsXML,
                                     ErrorStatistics&) const;
    ...
    virtual XMLElem convertRadiometryToXML(const Radiometric* obj,
    ...
    virtual XMLElem convertMatchInformationToXML(
            const MatchInformation& matchInfo, XMLElem parent) const = 0;
...
private:
    ...
    void parsePoly(const xml::lite::Element* polyXML,
                   size_t xyzIdx,
                   PolyXYZ& polyXYZ) const;

    XMLElem createFromElement(const six::XsElement<double>&,
                              xml::lite::Element&) const;

    template <typename T>
    XMLElem newFromElement(const XsElement_minOccurs0<T>& element,
