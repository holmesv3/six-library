...
void XMLControlRegistry::addCreator(
...
XMLControl* XMLControlRegistry::newXMLControl(const std::string& identifier,
                                              logging::Logger* log) const
{
    ...
    if (iter == mRegistry.end())
    {
        throw except::NoSuchKeyException(
                Ctxt("No data class creator " + identifier));
    }
                                   ...
                                   const six::XMLControlRegistry* xmlRegistry)
{
    ...
    const std::unique_ptr<XMLControl> xmlControl(
            xmlRegistry->newXMLControl(data.getDataType(), log));
...
std::u8string six::toValidXMLString(
        const Data& data,
