...
NITFWriteControl::NITFWriteControl(
        ...
        FILE* log /*= stderr*/)
{
    mNITFHeaderCreator.reset(
            new six::NITFHeaderCreator(options, container, log));
    if (xmlRegistry)
    {
...
NITFWriteControl::NITFWriteControl(const six::Options& options,
                                   std::shared_ptr<Container> container,
                                   FILE* log) :
    NITFWriteControl(options, container, nullptr /*xmlRegistry*/, log)
{
}
NITFWriteControl::NITFWriteControl(const six::Options& options,
                                   std::shared_ptr<Container> container,
...
bool NITFWriteControl::shouldByteSwap() const
{
    const int byteSwapping =
            (int)getOptions().getParameter(six::WriteControl::OPT_BYTE_SWAP,
                                           Parameter(static_cast<int>(
                                                   ByteSwapping::SWAP_AUTO)));
...
static auto asBytes(BufferList::value_type pImageData,
                    const NITFSegmentInfo& segmentInfo,
                    const Data& data)
{
    const auto bandSize = getBandSize(segmentInfo, data);
    auto size_in_bytes = bandSize * data.getNumChannels();

    // At this point, we've lost information about the ACTUAL size of the
    // buffer. Normally, the computation above will be correct. But in the case
    // of AMP8I_PHS8I (now supported), the buffer is actually RE32F_IM32F as the
    // data is converted to six::zfloat when read-in, and converted to
    // AMP8I_PHS8I_t when written-out.
    ...
    return sys::make_span(static_cast<const std::byte*>(pImageData_),
                          size_in_bytes);
}

// this bypasses the normal NITF ImageWriter and streams directly to the output
template <typename T>
inline std::shared_ptr<NewMemoryWriteHandler> makeWriteHandler(
        const NITFSegmentInfo& segmentInfo,
...
inline std::shared_ptr<NewMemoryWriteHandler> makeWriteHandler(
        const NITFSegmentInfo& segmentInfo,
        BufferList::value_type pImageData,
        const Data& data,
        bool doByteSwap)
{
...
inline std::shared_ptr<StreamWriteHandler> makeWriteHandler(
        NITFSegmentInfo segmentInfo,
        io::InputStream* imageData,
        const Data& data,
        bool doByteSwap)
{
...
void writeWithoutNitro(nitf::Writer& mWriter,
                       const TImageData& imageData,
                       ...
                       bool doByteSwap)
{
    ...
    {
        auto writeHandler =
                makeWriteHandler(imageSegments[j], imageData, data, doByteSwap);
        mWriter.setImageWriteHandler(static_cast<int>(startIndex + j),
                                     writeHandler);
    }
}

bool NITFWriteControl::do_prepareIO(size_t imageDataSize,
...
static nitf::ImageSource make_ImageSource_(std::span<const std::byte> pData,
                                           size_t numChannels,
                                           size_t pixelSize)
{
...
template <typename T>
static nitf::ImageSource do_make_ImageSource(std::span<const T> pImageData_,
                                             const NITFSegmentInfo& segmentInfo,
                                             const Data& data)
{
    ...
    const auto numCols = data.getNumCols();
    const auto pData_ =
            pImageData.data() + pixelSize * segmentInfo.getFirstRow() * numCols;
    const std::span<const std::byte> pData(pData_, bandSize);
    return make_ImageSource_(pData, numChannels, pixelSize);
}
template <typename T>
static nitf::ImageSource make_ImageSource(std::span<const T> pImageData,
                                          const NITFSegmentInfo& segmentInfo,
                                          const Data& data)
{
...
// Existing code that uses BufferList needs raw "std::byte*" instead of
// std::span<std::byte>
inline nitf::ImageSource make_ImageSource_from_BufferList(
        std::span<const std::byte> pImageData,
        const NITFSegmentInfo& segmentInfo,
        const Data& data)
{
    // The pixelSize calculation value probably won't match
    // sizof(BufferList::value_type) which is "unsigned char"
    return do_make_ImageSource(pImageData, segmentInfo, data);
}
static nitf::ImageSource make_ImageSource(
        const BufferList::value_type pImageData,
        const NITFSegmentInfo& segmentInfo,
        const Data& data)
{
    const auto pImageData_ = asBytes(pImageData, segmentInfo, data);
...
template <typename TImageData>
void writeWithNitro(nitf::Writer& mWriter,
                    const std::map<std::string, void*>& mCompressionOptions,
                    const TImageData& imageData,
                    const std::vector<NITFSegmentInfo>& imageSegments,
                    size_t startIndex,
                    const Data& data)
{
    for (size_t jj = 0; jj < imageSegments.size(); ++jj)
    {
        ...
        nitf::ImageWriter iWriter =
                mWriter.newImageWriter(static_cast<int>(startIndex + jj),
                                       mCompressionOptions);
...
void NITFWriteControl::addLegend(const Legend& legend, int imageNumber)
{
    ...
    const std::span<const std::byte> image(static_cast<const std::byte*>(
                                                   pImage),
                                           legend.mImage.size());
    nitf::MemorySource memSource(image,
                                 0 /*start*/,
                                 1 /*numBytesPerPixel*/,
                                 0 /*pixelSkip*/);
...
template <typename T>
bool NITFWriteControl::prepareIO(const T& imageData,
                                 nitf::IOInterface& outputFile)
{
...
template <typename T>
void NITFWriteControl::write_imageData(const T& imageData,
                                       ...
                                       bool enableJ2K)
{
    ...
    const bool isBlocking =
            subheader.numBlocksPerRow() > 1 || subheader.numBlocksPerCol() > 1;
    ...
    if (isBlocking || (enableJ2K && numIS == 1) || !mCompressionOptions.empty())
    {
        if ((isBlocking || (enableJ2K && numIS == 1)) &&
            (pData->getDataType() == six::DataType::COMPLEX))
        {
            throw except::Exception(Ctxt(
                    "SICD does not support blocked or J2K compressed output"));
        }

        writeWithNitro(mWriter,
                       mCompressionOptions,
                       imageData,
                       imageSegments,
                       startIndex,
...
template <typename T>
void NITFWriteControl::do_save(const T& imageData,
                               nitf::IOInterface& outputFile,
                               const std::vector<std::string>& schemaPaths)
{
...
template <>
void NITFWriteControl::do_save_(const BufferList& list,
                                bool doByteSwap,
                                bool enableJ2K)
{
...
void NITFWriteControl::save_buffer_list(
...
template <typename T>
void NITFWriteControl::do_save_(const T& imageData,
                                bool doByteSwap,
                                bool enableJ2K)
{
    size_t numImages = getInfos().size();
    if (numImages > 1)
    {
        throw std::invalid_argument(
                "Should only have one image!");  // we would be in
                                                 // do_save_(BufferList)
    }
    if (numImages == 1)  // can we really have 0 images?
    {
        constexpr size_t i =
                0;  // keep code consistent with do_save_(BufferList)
...
void NITFWriteControl::save_image(
        std::span<const six::zfloat> imageData,
        nitf::IOInterface& outputFile,
...
void NITFWriteControl::save_image(
        std::span<const six::zint16_t> imageData,
        nitf::IOInterface& outputFile,
        const std::vector<std::filesystem::path>& schemaPaths)
{
    do_save(imageData, outputFile, sys::convertPaths(schemaPaths));
...
void NITFWriteControl::save_image(
        std::span<const uint16_t> imageData,
        nitf::IOInterface& outputFile,
        const std::vector<std::filesystem::path>& schemaPaths)
{
    do_save(imageData, outputFile, sys::convertPaths(schemaPaths));
...
void NITFWriteControl::save(const BufferList& list,
                            const std::string& outputFile,
                            const std::vector<std::string>& schemaPaths)
{
...
void save(NITFWriteControl& writeControl,
          const six::zfloat* image,
          const std::string& outputFile,
          const std::vector<std::string>& schemaPaths)
{
    // Keeping this code-path in place as it's an easy way to test legacy
    // BufferList functionality.
        ...
        const std::vector<std::string>& schemaPaths)
{
    ...
    for (size_t ii = 0; ii < numDES; ++ii)
    {
        ...
        nitf::SegmentWriter deWriter =
                mWriter.newDEWriter(gsl::narrow<int>(ii));
