...
static inline void byteSwap(std::vector<std::byte>& buffer,
                            size_t elemSize,
                            size_t numElems)
{
...
static NITF_BOOL write(const TImpl* impl,
                       nitf_IOInterface* io,
                       nitf_Error* error,
                       TWriteFunc write_f)
{
    ...
    for (unsigned int i = 0; i < impl->numRows; i++)
    {
        ...
        if (impl->doByteSwap)
            byteSwap(rowCopy,
                     impl->pixelSize / impl->numChannels,
                     impl->numCols * impl->numChannels);
...
template <typename TPImpl>
static nitf_SegmentWriter* create_SegmentWriter(
        TPImpl impl, nitf_IWriteHandler& iWriteHandler)
{
                                       ...
                                       bool doByteSwap)
{
    ...
    static nitf_IWriteHandler iWriteHandler = {
            &six_MemoryWriteHandler_write, &six_MemoryWriteHandler_destruct};
    auto segmentWriter = create_SegmentWriter(info,
                                              buffer,
                                              firstRow,
                                              numCols,
                                              numChannels,
...
template <typename T>
inline void validate_bandSize(std::span<T> buffer,
                              const NITFSegmentInfo& info,
                              const Data& data)
{
    ...
    {
        // This is not always correct: the sizes are computed using the values
        // in NITFSegmentInfo/Data but we may be working with other data.  This
        // is the case when we have six::zfloat data that will be converted to
        // AMP8I_PHS8I when written to disk.
        // throw std::invalid_argument("buffer.size()!");    }
    }
}

template <typename T>
inline void validate_buffer(std::span<T> buffer,
                            const NITFSegmentInfo& info,
                            const Data& data)
{
...
struct NewMemoryWriteHandler::Impl final
{
    ...
    void convertPixels(NewMemoryWriteHandler& instance,
                       const NITFSegmentInfo& info,
    ...
    NewMemoryWriteHandler(info, cast(buffer), firstRow, data, doByteSwap)
{
    ...
    if (data.getPixelType() == six::PixelType::AMP8I_PHS8I)
    {
        ...
        const std::span<const six::zfloat> buffer_(pBuffer,
                                                   buffer.size() /
                                                           sizeof(six::zfloat));
...
NewMemoryWriteHandler::NewMemoryWriteHandler(
        const NITFSegmentInfo& info,
        std::span<const six::zfloat> buffer,
        size_t firstRow,
        const Data& data,
...
NewMemoryWriteHandler::NewMemoryWriteHandler(
    ...
    NewMemoryWriteHandler(info, cast(buffer), firstRow, data, doByteSwap)
{
    // This is for the uncommon case where the data is already in this format;
    // normally, it is six::zfloat.
    if (data.getPixelType() != six::PixelType::AMP8I_PHS8I)
    {
        throw std::invalid_argument("pixelType is wrong.");
    }
    ...
    NewMemoryWriteHandler(info, cast(buffer), firstRow, data, doByteSwap)
{
    ...
    // imaginary components. Each component is stored in a 16-bit signed integer
    // in 2’s complement format (2 bytes per component, 4 bytes per pixel).
                                       ...
                                       bool doByteSwap)
{
    ...
    static nitf_IWriteHandler iWriteHandler = {
            &six_StreamWriteHandler_write, &six_StreamWriteHandler_destruct};
...
StreamWriteHandler::StreamWriteHandler(const NITFSegmentInfo& info,
                                       ...
                                       const Data& data,
                                       bool doByteSwap) :
    StreamWriteHandler(info,
                       is,
                       data.getNumCols(),
                       data.getNumChannels(),
                       data.getNumBytesPerPixel() / data.getNumChannels(),
                       doByteSwap)
{
}
